...
const initialState = {
    ...
    user: {
        ...
        name: null,
        city: null,
        active: false, // ברירת מחדל ל-false
        currentOrder: null,
    }
};

const tokenSlice = createSlice({
    name: 'auth', // השם של ה-slice
    initialState,
    reducers: {
        setAuth: (state, action) => {
            // action.payload הוא האובייקט המלא שמתקבל מהשרת (accessToken, user)
            state.accessToken = action.payload.accessToken || null;

            // עדכון אובייקט ה-user מתוך action.payload.user
            state.user = {
                _id: action.payload.user?._id || null,
                username: action.payload.user?.username || null,
                // **תיקונים חשובים כאן:**
                role: action.payload.user?.roles || null, // לקרוא מתוך user.roles
                name: action.payload.user?.name || null, // לקרוא מתוך user.name
                city: action.payload.user?.city || null, // לקרוא מתוך user.city
                active: action.payload.user?.active !== undefined ? action.payload.user.active : false, // לקרוא מתוך user.active
                currentOrder: action.payload.user?.currentOrder || null, // אם השרת שולח currentOrder בתוך user
            };
        },
        setDeliverActiveStatus: (state, action) => {
            // פעולה ייעודית לשינוי סטטוס active של משלוחן
            state.user.active = action.payload.active;
            state.user.currentOrder = action.payload.currentOrder || null;
        },
        clearAuth: (state) => {
